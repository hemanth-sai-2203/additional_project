...
BUG FIX (this version):
  _predict_core covariance update was written as:
      self.P = 0.5*(F@self.P@F.T+Q+(F@self.P@F.T+Q).T)/2
  Python evaluates * and / left-to-right with equal precedence, so this is:
      (0.5 * (M + M.T)) / 2  =  (M + M.T) / 4
  which divides by 4 instead of 2, under-inflating uncertainty by 4x every
  prediction step and making the filter chronically overconfident during
  dead-reckoning.  Fix: compute M once, then 0.5*(M + M.T).
...
from lstm.train_lstm_physics import PhysicsBiasPredictor
...
# FIX-E: Reduced from 4.0 -> 1.5 for v and psi. The original 4x inflation caused
# the EKF to spread 4x faster than training, making LSTM corrections diverge.
GPS_DENIED_Q_SCALE = np.array([1.0, 1.0, 1.5, 1.5, 1.0], dtype=np.float64)
...
class LSTMBridge:
    ...
    def __init__(self, model_path, stats_path, device='cpu'):
        ...
        self.model = PhysicsBiasPredictor(
            input_size=cfg.get('input_size', 3),
            output_size=3,
            h1=cfg.get('h1', 128), h2=cfg.get('h2', 64),
            dropout=0.3,
        ).to(self.device)
        self.model.load_state_dict(ckpt['model_state'])
        self.model.eval()
        self._loaded = True
        print(f"  [LSTMBridge] Physics-Aware SOTA loaded.")
        print(f"  [LSTMBridge] Application: Corrects a_fwd, a_lat, and wz!")
    ...
    def push(self, ax_corr, ay_corr, wz, ekf_speed, gps_denied):
        # Explicitly drop ekf_speed and gps_denied!
        raw = np.array([ax_corr, ay_corr, wz], dtype=np.float32)
        if self.feat_mean is not None:
            raw = (raw - self.feat_mean) / self.feat_std
        self._buffer.append(raw)
        if len(self._buffer) > SEQ_LEN: self._buffer.popleft()

    def predict(self):
        """
        Returns (bias_fwd, bias_lat, bias_wz, var_fwd, var_lat, var_wz)
        """
        if not self.ready(): return None, None, None, None, None, None
        seq = np.array(self._buffer, dtype=np.float32)
        x_t = torch.from_numpy(seq).unsqueeze(0).to(self.device)
        with torch.no_grad():
            mean_pred, log_var = self.model(x_t)
            mu_n = mean_pred.cpu().numpy()[0]
            lv   = log_var.cpu().numpy()[0]
            
        biases = mu_n * self.tgt_std + self.tgt_mean
        vars_norm = np.exp(np.clip(lv, -5.0, 5.0))
        vars_val = vars_norm * (self.tgt_std ** 2)
        
        return (float(biases[0]), float(biases[1]), float(biases[2]),
                float(vars_val[0]), float(vars_val[1]), float(vars_val[2]))
...
class AdaptiveEKF:
    ...
    BUG FIX in _predict_core (this version):
      Old (wrong):
          self.P = 0.5*(F@self.P@F.T+Q+(F@self.P@F.T+Q).T)/2
          # Python evaluates * and / left-to-right:
          #   = (0.5 * (M + M.T)) / 2  =  (M + M.T) / 4   <-- divided by 4!
          # This under-inflates P by 4x every prediction step, making the
          # filter chronically overconfident during dead-reckoning.

      New (correct):
          M = F @ self.P @ F.T + Q
          self.P = 0.5 * (M + M.T)       # proper symmetrisation: (M+M.T)/2
    """
    ...
    def _predict_core(self, a_fwd, wz, gps_denied):
        ...
        # BUG FIX: old code was 0.5*(M + M.T)/2 = (M + M.T)/4  (wrong).
        # Python evaluates * and / left-to-right at equal precedence, so
        # 0.5 * (...) / 2  is  (0.5 * ...) / 2  =  ... / 4.
        # Correct symmetrisation is (M + M.T) / 2 = 0.5 * (M + M.T).
        M = F @ self.P @ F.T + Q
        self.P = 0.5 * (M + M.T)
    ...
    def update_pseudo_velocity(self, v_meas, r_variance=1.0):
        self._assert_initialized()
        H = np.zeros((1, N_STATES), dtype=np.float64)
        H[0, IDX_V] = 1.0
        R = np.array([[float(max(r_variance, 1e-6))]], dtype=np.float64)
...
def run_ekf_on_run(df_run, ekf, bridge, use_lstm):
    ...
    for _, row in df_run.iterrows():
        ...
        # Wheel odometry (new data only - backward compatible with old data)
        odom_v = None
        odom_raw = row.get('odom_speed_mps', float('nan'))
        if not pd.isna(odom_raw):
            odom_v = float(odom_raw)
        ...
        if bridge is not None:
            # We pass ekf_speed, but the bridge now drops it internally!
            bridge.push(ax_corr, ay_corr, wz, ekf.get_speed(), gps_denied)
            if use_lstm and gps_denied and bridge.ready():
                bf, bl, bpsi, vf, vl, vpsi = bridge.predict()
                if bf is not None:
                    a_fwd_used = ax_corr + bf
                    # Correct heading drift dynamically!
                    wz_used    = wz + bpsi
                    lstm_used = True
                    
                    # Dynamically inflate process noise based on LSTM uncertainty
                    # Q_v scales by the variance of the forward bias
                    # Base Q_scale is 1.0. If uncertainty is high, this spikes.
                    ekf.set_process_noise_scale([1, 1, 1 + vf, 1 + vpsi, 1])

        ekf.predict(a_fwd=a_fwd_used, wz=wz_used, gps_denied=bool(gps_denied))

        # --- WHEEL ODOMETRY UPDATE (Block 6 key upgrade) ---
        # When GPS is denied, use wheel odometry to anchor speed.
        # This is the single biggest drift killer — keeps velocity error near 0.
        if gps_denied and odom_v is not None:
            ekf.update_pseudo_velocity(odom_v, r_variance=0.1)  # tight: std=0.32 m/s
...
# =============================================================================
# SELF-TEST  (verifies basic EKF math, including the P growth fix)
# =============================================================================
def _run_self_test():
    ...
    # PASS 5: verify P grows monotonically during GPS denial (catches the /4 bug).
    trace_p_at_deny_start = None
    trace_p_at_deny_end   = None
    ...
    for k in range(n_steps):
        ...
        if k == DENY_START:
            trace_p_at_deny_start = ekf.trace_P
        ...
        if k == DENY_END - 1:
            trace_p_at_deny_end = ekf.trace_P
    ...
    mr=float(np.mean(errors_road)); assert mr<2.0; print(f"  PASS 1/5  Road mean {mr:.4f} m"); pass_count+=1
    if errors_tunnel: ft=errors_tunnel[-1]; assert ft<15.0; print(f"  PASS 2/5  Tunnel final {ft:.4f} m"); pass_count+=1
    print("  PASS 3/5  P symmetric PD every step"); pass_count+=1
    ekf.set_noise_scales(2.0,2.0); s=ekf.get_state()
    assert abs(s['Q_scale']-2.0)<1e-6; print(f"  PASS 4/5  RL hook round-trip"); pass_count+=1

    # PASS 5: P trace must be strictly larger at end of denial than at start.
    # The /4 bug caused P to grow only 1/4 as fast, so this catches it when
    # combined with a process noise that should significantly inflate P.
    assert trace_p_at_deny_end > trace_p_at_deny_start, (
        f"FAIL 5/5  P did not grow during GPS denial: "
        f"trace_start={trace_p_at_deny_start:.6f}  trace_end={trace_p_at_deny_end:.6f}. "
        f"Check _predict_core covariance symmetrisation."
    )
    print(f"  PASS 5/5  P grows during denial  "
          f"(trace: {trace_p_at_deny_start:.4f} → {trace_p_at_deny_end:.4f})")
    pass_count+=1
