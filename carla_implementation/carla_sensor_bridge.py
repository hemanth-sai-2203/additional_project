...
class GroundTruth:
    ...
    odom_speed: float = 0.0       # Forward speed (m/s) from wheel rotation
...
class GPSDenialManager:
    ...
    def is_gps_denied(self, x: float, y: float,
        ...
        if self.method in ("zone", "both"):
            # Fix: Check zones using CARLA world coordinates, not local EKF coordinates
            if self._check_zones(carla_location.x, carla_location.y):
                return True
...
class CARLASensorBridge:
    ...
    def _attach_spectator(self):
        ...
        try:
            ...
            # Position 25m above vehicle and 25m behind, looking down at 45 deg (centers the car)
            spectator_loc = carla.Location(
                x=transform.location.x - 25.0 * transform.get_forward_vector().x,
                y=transform.location.y - 25.0 * transform.get_forward_vector().y,
    ...
    def get_ground_truth(self) -> GroundTruth:
        ...
        gt = GroundTruth(
            ...
            odom_speed = speed # In CARLA we use velocity as proxy for ideal odometry
        )
        self._last_gt = gt
        return gt
    ...
    def draw_3d_point(self, x_local, y_local, color='red', size=0.1, life_time=0.1):
        """Draw a colored sphere in the CARLA world at local coordinate (x,y)."""
        if not self.world or not self.origin_set: return
        
        # 1. Convert local EKF back to CARLA world
        # dx = x - ox => x = dx + ox
        # dy = - (y - oy) => y = -dy + oy
        cw_x = x_local + self.coord_conv.origin_x
        cw_y = -y_local + self.coord_conv.origin_y
        
        # 2. Get ground height at that location (approximate)
        loc = carla.Location(x=cw_x, y=cw_y, z=self.vehicle.get_location().z)
        
        # 3. Choose color
        c = carla.Color(255, 0, 0) # Default red
        if color == 'green': c = carla.Color(0, 255, 0)
        elif color == 'blue':  c = carla.Color(0, 0, 255)
        elif color == 'yellow': c = carla.Color(255, 255, 0)
        
        self.world.debug.draw_point(loc + carla.Location(z=0.5), size=size, color=c, life_time=life_time)
