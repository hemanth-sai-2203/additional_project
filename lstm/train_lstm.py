...
BUG FIXES (this version):
  FIX A — matplotlib backend changed from "TkAgg" to "Agg".
           "TkAgg" requires a Tk/display environment and raises
           "_tkinter.TclError: no display name and no $DISPLAY environment
           variable" in headless/server runs.  "Agg" is a file-only backend
           that works everywhere and is consistent with ekf.py.

  FIX B — plot_results: plt.show() replaced with plt.close(fig).
           plt.show() blocks execution until the window is manually closed,
           which is wrong for automated pipelines.  With the "Agg" backend
           it also silently does nothing, so the figure was never closed and
           would leak memory.  plt.close(fig) releases the figure
           immediately and does not block.

  FIX C — plot_results fill_between: ax_ts.get_ylim() was called before
           matplotlib had finished autoscaling the axis, so it returned the
           default (0.0, 1.0) limits rather than the actual data range.
           Fix: compute ymin/ymax directly from the plotted arrays so the
           shaded tunnel region always covers the correct data extent.
...
import pandas as pd
import matplotlib
# FIX A: use 'Agg' (file-only, no display required) instead of "TkAgg".
# "TkAgg" raises TclError in headless/server environments.  'Agg' is
# consistent with ekf.py and works everywhere.
matplotlib.use("Agg")
...
class IMUSequenceDataset(Dataset):
    def __init__(self, df_norm, preprocessor, stride=1):
        ...
        for i in range(0, n - SEQ_LEN, stride):
            ...
            if end >= n:                         continue
            if run_ids[i] != run_ids[end]:       continue   # no cross-run sequences
            x = features[i:end]
            y = targets[end]
...
def plot_results(train_losses, val_losses, pred_p, true_p, is_tun, save_path):
    ...
    ax_ts.plot(ts, true_p[:n,0],'g-',lw=1,label='True bias')
    ax_ts.plot(ts, pred_p[:n,0],'b--',lw=1,label='Pred bias')
    ax_ts.set_title("Fwd Bias Time Series (25s)"); ax_ts.legend(fontsize=7); ax_ts.grid(True,alpha=0.3)
    if is_tun[:n].any():
        # FIX C: compute ymin/ymax from the actual plotted data instead of
        # calling ax_ts.get_ylim() before matplotlib has finished autoscaling.
        # ax_ts.get_ylim() at this point returns the default (0.0, 1.0) limits
        # because autoscale fires lazily on render, not on plot().  Reading from
        # the data arrays directly always produces the correct shaded region.
        data_min = min(true_p[:n,0].min(), pred_p[:n,0].min())
        data_max = max(true_p[:n,0].max(), pred_p[:n,0].max())
        margin   = (data_max - data_min) * 0.05
        ax_ts.fill_between(ts, data_min - margin, data_max + margin,
                           where=is_tun[:n], alpha=0.15, color='red', label='Tunnel')
    ...
    # FIX B: plt.show() was replaced with plt.close(fig).
    # plt.show() blocks execution until the window is manually closed, which
    # breaks automated pipelines.  With the 'Agg' backend it also silently
    # does nothing while leaving the figure open in memory.  plt.close(fig)
    # releases the figure immediately without blocking.
    plt.close(fig)
